...
USERNAME=$(whoami)
VENV_PATH="/workspace-vast/${USERNAME}/envs/.penv"
...
while getopts "g:c:m:v:h" opt; do
    case $opt in
        ...
        v) VENV_PATH="$OPTARG" ;;
        h)
            echo "Usage: $0 [-g gpus] [-c cpus] [-m mem] [-v venv_path]"
            ...
            echo "  -v <path>    Path to venv (default: /workspace-vast/\$USER/envs/.penv)"
...
python -m ipykernel install --user --name yoenv --display-name \"yoenv\"
